    ...
    bool upper_bound_inclusive)
{
    ...
    std::function<void(typename search_tree<tkey, tvalue>::common_node*)> traverse_and_collect;
    traverse_and_collect = [&](typename search_tree<tkey, tvalue>::common_node *node)
    {
        if (!node) return;

        for (size_t i = 0; i < node->virtual_size; ++i)
        {
            traverse_and_collect(node->subtrees[i]);

            if ((lower_bound_inclusive ? node->keys_and_values[i].key >= lower_bound : node->keys_and_values[i].key > lower_bound) &&
                (upper_bound_inclusive ? node->keys_and_values[i].key <= upper_bound : node->keys_and_values[i].key < upper_bound))
            {
                result.push_back(node->keys_and_values[i]);
            }

            if (node->keys_and_values[i].key > upper_bound && !upper_bound_inclusive)
            {
                return;
            }
        }

        traverse_and_collect(node->subtrees[node->virtual_size]);
    };

    traverse_and_collect(this->_root);
